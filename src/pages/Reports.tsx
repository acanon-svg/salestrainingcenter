...
import {
  useLeaderReportUserIds,
  useHierarchyOverviewStats,
  useHierarchyTeamData,
  useHierarchyTrendData,
...
const LeaderReportsView: React.FC<{ dateRange: string }> = ({ dateRange }) => {
  const { data: leaderData, isLoading: loadingLeaderData } = useLeaderReportUserIds();
  const { data: hierarchyOverview, isLoading: loadingHierarchyOverview } = useHierarchyOverviewStats({
    dateRange: parseInt(dateRange),
  });
  const { data: hierarchyTeamData, isLoading: loadingHierarchyTeams } = useHierarchyTeamData({
    dateRange: parseInt(dateRange),
  });
  const { data: hierarchyTrendData, isLoading: loadingHierarchyTrend } = useHierarchyTrendData({
    dateRange: parseInt(dateRange),
  });
  ...
  if (!leaderData || !hierarchyOverview) {
    return (
      <Card className="border-border/50">
        <CardContent className="pt-6 text-center py-12">
          <AlertCircle className="w-12 h-12 mx-auto mb-4 text-muted-foreground opacity-50" />
          <p className="text-muted-foreground">No hay datos disponibles para tu equipo.</p>
        </CardContent>
      </Card>
    );
  }

  const isSupervisor = leaderData.isSupervisor;
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-semibold text-lg">
                {isSupervisor ? "Vista de Supervisor" : "Vista de Líder"}
              </h3>
              <p className="text-sm text-muted-foreground">
                {isSupervisor
                  ? `Viendo reportes de ${leaderData.subordinateLeaders.length} líderes subordinados (${leaderData.teams.length} equipos)`
                  : `Viendo reportes de tu equipo: ${hierarchyOverview.teamName}`}
              </p>
            </div>
          </div>
        </CardContent>
        ...
        <Card className="border-border/50">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-2xl font-bold">{hierarchyOverview.totalCourses}</p>
        ...
        <Card className="border-border/50">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-2xl font-bold">{hierarchyOverview.activeUsers}</p>
        ...
        <Card className="border-border/50">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-2xl font-bold">{hierarchyOverview.totalEnrollments}</p>
        ...
        <Card className="border-border/50">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-2xl font-bold">{hierarchyOverview.avgCompletionRate}%</p>
        ...
        <Card className="border-border/50">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-2xl font-bold">{hierarchyOverview.avgScore}%</p>
        ...
        <Card className="border-border/50">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-2xl font-bold">+{hierarchyOverview.completedThisPeriod}</p>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="border-border/50">
          <CardHeader>
            <CardTitle>Compliance por Equipo</CardTitle>
            <CardDescription>
              {isSupervisor ? "Desempeño de equipos de tus líderes subordinados" : "Desempeño de tu equipo"}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {loadingHierarchyTeams ? (
              <Skeleton className="h-64 w-full" />
            ) : hierarchyTeamData && hierarchyTeamData.length > 0 ? (
              <div className="space-y-3">
                {hierarchyTeamData.map((team) => (
                  <div key={team.team} className="space-y-2">
                    <div className="flex items-center justify-between">
                      <div>
                        <span className="font-medium">{team.team}</span>
                        {team.subordinate_name && (
                          <span className="text-xs text-muted-foreground ml-2">
                            (Líder: {team.subordinate_name})
                          </span>
                        )}
                      </div>
                      <Badge
                        className={
                          team.completion_rate >= 80
                            ? "bg-success"
                    ...
                    <div className="flex gap-4 text-xs text-muted-foreground">
                      <span>{team.users} usuarios</span>
                      <span>{team.courses_completed} completados</span>
                      <span>{team.avg_score}% promedio</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {/* Status Distribution */}
        <StatusPieChart data={hierarchyStatusData} isLoading={loadingHierarchyStatus} />
      </div>

      {/* Trend Chart */}
      <TrendLineChart data={hierarchyTrendData} isLoading={loadingHierarchyTrend} dateRange={dateRange} />

      {/* Subordinate Leaders List (only for supervisors) */}
      {isSupervisor && leaderData.subordinateLeaders.length > 0 && (
        <Card className="border-border/50">
          <CardHeader>
            ...
            <CardDescription>Líderes bajo tu supervisión y sus equipos</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {leaderData.subordinateLeaders.map((leader) => {
                const leaderTeamData = hierarchyTeamData?.find((t) => t.team === leader.team);
                return (
                  <div key={leader.user_id} className="p-4 rounded-lg border border-border/50">
                    <div className="flex items-center gap-3 mb-3">
                      <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
                        <UserCheck className="w-5 h-5 text-primary" />
                      </div>
                      <div>
                        <p className="font-medium">{leader.full_name || "Sin nombre"}</p>
                        <p className="text-xs text-muted-foreground">{leader.team || "Sin equipo"}</p>
                      </div>
                    </div>
                    {leaderTeamData && (
                      <div className="space-y-2">
                        <div className="flex items-center justify-between text-sm">
                          ...
                          <Badge
                            className={
                              leaderTeamData.completion_rate >= 80
                                ? "bg-success"
                                : leaderTeamData.completion_rate >= 60
                                ? "bg-addi-yellow text-black"
                                : "bg-destructive"
                            }
                          >
                            {leaderTeamData.completion_rate}%
                          </Badge>
                        </div>
                        <Progress
                          value={leaderTeamData.completion_rate}
                          className={`h-1.5 ${
                            leaderTeamData.completion_rate >= 80
                              ? "[&>div]:bg-success"
                              : leaderTeamData.completion_rate >= 60
                        ...
                        <div className="flex justify-between text-xs text-muted-foreground">
                          <span>{leaderTeamData.users} miembros</span>
                          <span>{leaderTeamData.avg_score}% promedio</span>
                        </div>
                      </div>
                    )}
                  </div>
...
const Reports: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<"leader" | "global">(isLeader && !isAdminOrAnalyst ? "leader" : "global");
  ...
  return (
    <DashboardLayout>
      <div className="space-y-8 animate-fade-in">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              {viewMode === "leader"
                ? "Dashboard de desempeño de tu equipo"
                : "Dashboard de desempeño y compliance del equipo"}
            </p>
          </div>

          <div className="flex items-center gap-2">
            {/* View Mode Toggle (for leaders who also have admin/analyst role) */}
            {isLeader && isAdminOrAnalyst && (
              <Select value={viewMode} onValueChange={(v) => setViewMode(v as "leader" | "global")}>
        ...
        {/* Date Range Filter (common for both views) */}
        <Card className="border-border/50">
          <CardContent className="pt-6">
        ...
        {viewMode === "leader" ? (
          <LeaderReportsView dateRange={dateRange} />
        ) : (
          <>
                      ...
                      <div className="space-y-4">
                        {regionalData.map((regional) => (
                          ...
                          >
                            ...
                            <div className="grid gap-4 md:grid-cols-3">
                              <div>
                                <p className="text-sm text-muted-foreground mb-1">Cursos Completados</p>
                                <p className="text-xl font-bold">{regional.courses_completed}</p>
                              </div>
                              <div>
                                <p className="text-sm text-muted-foreground mb-1">Promedio Calificación</p>
                                <div className="flex items-center gap-2">
